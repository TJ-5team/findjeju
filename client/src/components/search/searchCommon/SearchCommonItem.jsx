...
export default function SearchCommonItem({list}) {
  ...
  return (
      <ul className={styles.courseList}>
        <li>
          <Link className={styles.searchImg} to={`/detail/${list.contentid}/${list.contenttypeid}`}>
          ...
          <div className={styles.commonContents}>
            ...
            <span className={styles.contentsArea}>{list.addr1 && list.addr1.split(" ")[0]} {list.addr1 && list.addr1.split(" ")[1]} {list.addr1 === "" && "제주특별자치도"}</span>
            {list.contenttypeid === "25" && <div className={styles.courseTag}>
              <span>
                여행코스
              </span>
            </div>}
